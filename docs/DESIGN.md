...
| 7 | Web interface | **Done.** Config layering, settings/diagnostics UI, Argon2id auth with a generated passphrase, CSRF, confirm-or-revert on display changes. The render and enrichment settings are *restart-required*, not live — see §7.3. |
| 8 | Provisioning | **Done.** `install.sh` (idempotent, `--dry-run`, `--uninstall`), a cross-buildable `.deb` with maintainer scripts, tmpfiles, the writable-partition and overlay helpers, a generated placeholder, and `docs/BUILD.md` from clean flash to working device. Cross-checked against the units and the schema by `crates/lpframe-config/tests/packaging.rs`. **Not yet run end to end on real hardware.** |
| 9 | *Bonus* | **Done.** A pinned `pi-gen` `arm64` stage that installs the cross-built `.deb` and then runs `install.sh` itself, so the image cannot drift from the documented install. Appends a labelled `/var/lib/lpframe` partition with `mke2fs -d` and `sfdisk` on a file — no loop device, no root, and therefore tested rather than argued for. Strips the `resize` token so first boot cannot grow root over that partition. No credentials of any kind are baked in. Built by `.github/workflows/image.yml` on a tag or on demand, which then mounts the result and checks it. **No image has been booted on hardware.** |
